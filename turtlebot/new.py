...
class DetectLevel():
    ...
    def fnFindRectOfLevel(self, mask):
        ...
        mean_x_2 = 0.0 # new line###################
        mean_x_2 = 0.0 # new line###################

        mean_x_3 = 0.0 # new line###################
        mean_y_3 = 0.0 # new line###################
...
################################################################################################
        #if detected 4 red rectangular 
        elif len(keypts) == 4:
            for i in range(3):
                mean_x = mean_x + keypts[i].pt[0]/3
                mean_y = mean_y + keypts[i].pt[1]/3

                mean_x_2 = mean_x_2 + keypts[i+1].pt[0]/3
                mean_y_2 = mean_y_2 + keypts[i+1].pt[1]/3
                
            arr_distances = [0]*3
            for i in range(3):
                arr_distances[i] = fnCalcDistanceDot2Dot(mean_x, mean_y, keypts[i].pt[0], keypts[i].pt[1])
                arr_distances_2[i] = fnCalcDistanceDot2Dot(mean_x_2, mean_y_2, keypts[i+1].pt[0], keypts[i+1].pt[1])                
            ...
            idx1_2, idx2_2, idx3_2 = fnArrangeIndexOfPoint(arr_distances)
            frame = cv2.line(frame, (int(keypts[idx1_2].pt[0]), int(keypts[idx1_2].pt[1])), (int(keypts[idx2_2].pt[0]), int(keypts[idx2_2].pt[1])), (255, 0, 0), 5)
            frame = cv2.line(frame, (int(mean_x_2), int(mean_y_2)), (int(mean_x_2), int(mean_y_2)), (255, 255, 0), 5)
            point1_2 =  [int(keypts[idx1_2].pt[0]), int(keypts[idx1_2].pt[1]-1)]
            point2_2 = [int(keypts[idx3_2].pt[0]), int(keypts[idx3_2].pt[1]-1)]
            point3_2 = [int(keypts[idx2_2].pt[0]), int(keypts[idx2_2].pt[1]-1)]

            # test linearity and distance equality. If satisfy the both, continue to next process
            is_rects_linear = fnCheckLinearity(point1, point2, point3)
            is_rects_dist_equal = fnCheckDistanceIsEqual(point1, point2, point3)

            is_rects_linear_2 = fnCheckLinearity(point1_2, point2_2, point3_2)
            is_rects_dist_equal_2 = fnCheckDistanceIsEqual(point1_2, point2_2, point3_2)            

            if (is_rects_linear == True or is_rects_dist_equal == True) and (is_rects_linear_2 == False and is_rects_dist_equal_2 == False):
                # finding the angle of line
                distance_bar2car = 100 / fnCalcDistanceDot2Dot(point1[0], point1[1], point2[0], point2[1])
                ...
                if distance_bar2car > 0.8:
                    ...
                    is_level_close = True
                    self.stop_bar_state = 'stop'
            elif (is_rects_linear_2 == True or is_rects_dist_equal_2 == True) and (is_rects_linear == False and is_rects_dist_equal == False):
                # finding the angle of line
                distance_bar2car = 100 / fnCalcDistanceDot2Dot(point1_2[0], point1_2[1], point2_2[0], point2_2[1])
                ...
                if distance_bar2car > 0.8:
                    ...
                    is_level_close = True
                    self.stop_bar_state = 'stop'
            elif (is_rects_linear == True or is_rects_dist_equal == True) and (is_rects_linear_2 == True or is_rects_dist_equal_2 == True):
                # finding the angle of line
                distance_bar2car = 100 / fnCalcDistanceDot2Dot(point1[0], point1[1], point2[0], point2[1])
                distance_bar2car_2 = 100 / fnCalcDistanceDot2Dot(point1_2[0], point1_2[1], point2_2[0], point2_2[1])

                if distance_bar2car > distance_bar2car_2:
                    distance_bar2car = distance_bar2car_2
                # publishing topic
                self.stop_bar_count = 40
                if distance_bar2car > 0.8:
                    is_level_detected = True
                    ...
                    self.stop_bar_state = 'stop'                    
###############################################################################################################
        #if detected 5 red rectangular 
        elif len(keypts) == 5:
            for i in range(3):
                mean_x = mean_x + keypts[i].pt[0]/3
                mean_y = mean_y + keypts[i].pt[1]/3

                mean_x_2 = mean_x + keypts[i+1].pt[0]/3
                mean_y_2 = mean_y + keypts[i+1].pt[1]/3
                
                mean_x_3 = mean_x_3 + keypts[i+2].pt[0]/3
                mean_y_3 = mean_y_3 + keypts[i+2].pt[1]/3
                
            arr_distances = [0]*3
            for i in range(3):
                arr_distances[i] = fnCalcDistanceDot2Dot(mean_x, mean_y, keypts[i].pt[0], keypts[i].pt[1])
                arr_distances_2[i] = fnCalcDistanceDot2Dot(mean_x_2, mean_y_2, keypts[i+1].pt[0], keypts[i+1].pt[1])
                arr_distances_3[i] = fnCalcDistanceDot2Dot(mean_x_3, mean_y_3, keypts[i+2].pt[0], keypts[i+2].pt[1])
            ...
            idx1_3, idx2_3, idx3_3 = fnArrangeIndexOfPoint(arr_distances)
            frame = cv2.line(frame, (int(keypts[idx1_3].pt[0]), int(keypts[idx1_3].pt[1])), (int(keypts[idx2_3].pt[0]), int(keypts[idx2_3].pt[1])), (255, 0, 0), 5)
            frame = cv2.line(frame, (int(mean_x_3), int(mean_y_3)), (int(mean_x_3), int(mean_y_3)), (255, 255, 0), 5)
            point1_3 =  [int(keypts[idx1_3].pt[0]), int(keypts[idx1_3].pt[1]-1)]
            point2_3 = [int(keypts[idx3_3].pt[0]), int(keypts[idx3_3].pt[1]-1)]
            point3_3 = [int(keypts[idx2_3].pt[0]), int(keypts[idx2_3].pt[1]-1)]            
            ...
            is_rects_linear_3 = fnCheckLinearity(point1_3, point2_3, point3_3)
            is_rects_dist_equal_3 = fnCheckDistanceIsEqual(point1_3, point2_3, point3_3)             

            if (is_rects_linear == True or is_rects_dist_equal == True) and (is_rects_linear_2 == False and is_rects_dist_equal_2 == False) and (is_rects_linear_3 == False and is_rects_dist_equal_3 == False):
                # finding the angle of line
                distance_bar2car = 100 / fnCalcDistanceDot2Dot(point1[0], point1[1], point2[0], point2[1])
                ...
                if distance_bar2car > 0.8:
                    ...
                    is_level_close = True
                    self.stop_bar_state = 'stop'
            elif (is_rects_linear_2 == True or is_rects_dist_equal_2 == True) and (is_rects_linear == False and is_rects_dist_equal == False) and (is_rects_linear_3 == False and is_rects_dist_equal_3 == False):
                # finding the angle of line
                distance_bar2car = 100 / fnCalcDistanceDot2Dot(point1_2[0], point1_2[1], point2_2[0], point2_2[1])
                ...
                if distance_bar2car > 0.8:
                    ...
                    is_level_close = True
                    self.stop_bar_state = 'stop'
            elif (is_rects_linear_3 == True or is_rects_dist_equal_3 == True) and (is_rects_linear_2 == False and is_rects_dist_equal_2 == False) and (is_rects_linear == False and is_rects_dist_equal == False):
                # finding the angle of line
                distance_bar2car = 100 / fnCalcDistanceDot2Dot(point1[0]_3, point1[1]_3, point2[0]_3, point2[1]_3)
                ...
                if distance_bar2car > 0.8:
                    ...
                    is_level_close = True
                    self.stop_bar_state = 'stop'
            elif (is_rects_linear == True or is_rects_dist_equal == True) and (is_rects_linear_2 == True or is_rects_dist_equal_2 == True) and (is_rects_linear_3 == False and is_rects_dist_equal_3 == False):
                # finding the angle of line
                distance_bar2car = 100 / fnCalcDistanceDot2Dot(point1[0], point1[1], point2[0], point2[1])
                distance_bar2car_2 = 100 / fnCalcDistanceDot2Dot(point1_2[0], point1_2[1], point2_2[0], point2_2[1])
                ...
                if distance_bar2car > 0.8:
                    ...
                    is_level_close = True
                    self.stop_bar_state = 'stop'
            elif (is_rects_linear == True or is_rects_dist_equal == True) and (is_rects_linear_3 == True or is_rects_dist_equal_3 == True) and (is_rects_linear_2 == False and is_rects_dist_equal_2 == False):
                # finding the angle of line
                distance_bar2car = 100 / fnCalcDistanceDot2Dot(point1[0], point1[1], point2[0], point2[1])
                distance_bar2car_3 = 100 / fnCalcDistanceDot2Dot(point1_3[0], point1_3[1], point2_3[0], point2_3[1])

                if distance_bar2car>distance_bar2car_3:
                    distance_bar2car = distance_bar2car_3
                ...
                if distance_bar2car > 0.8:
                    ...
                    is_level_close = True
                    self.stop_bar_state = 'stop'
            elif (is_rects_linear_2 == True or is_rects_dist_equal_2 == True) and (is_rects_linear_3 == True or is_rects_dist_equal_3 == True) and (is_rects_linear == False and is_rects_dist_equal == False):
                # finding the angle of line
                distance_bar2car_2 = 100 / fnCalcDistanceDot2Dot(point1_2[0], point1_2[1], point2_2[0], point2_2[1])
                distance_bar2car_3 = 100 / fnCalcDistanceDot2Dot(point1_3[0], point1_3[1], point2_3[0], point2_3[1])

                if distance_bar2car_2>distance_bar2car_3:
                    distance_bar2car = distance_bar2car_3
                else:
                    distance_bar2car = distance_bar2car_2
                ...
                if distance_bar2car > 0.8:
                    ...
                    is_level_close = True
                    self.stop_bar_state = 'stop'
            elif (is_rects_linear_2 == True or is_rects_dist_equal_2 == True) and (is_rects_linear_3 == True or is_rects_dist_equal_3 == True) and (is_rects_linear == True or is_rects_dist_equal == True):
                # finding the angle of line
                distance_bar2car = 100 / fnCalcDistanceDot2Dot(point1[0], point1[1], point2[0], point2[1])
                distance_bar2car_2 = 100 / fnCalcDistanceDot2Dot(point1_2[0], point1_2[1], point2_2[0], point2_2[1])
                distance_bar2car_3 = 100 / fnCalcDistanceDot2Dot(point1_3[0], point1_3[1], point2_3[0], point2_3[1])

                distance_bar2car = min([distance_bar2car,distance_bar2car_2,distance_bar2car_3]) ####################### is possible????????????????
